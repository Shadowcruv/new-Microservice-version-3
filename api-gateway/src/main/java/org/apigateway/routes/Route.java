...
public class Route {
    ...
    //Meanwhile this approach we use down is what is called FunctionEndpoints type of programming, it replaces the annotation type of programming where you could be using RequestMapping e.t.c
    @Bean
...
//        return route("fallbackRoute").GET("/fallback", request -> ServerResponse.status(HttpStatus.SERVICE_UNAVAILABLE).body("Service Unavailable, please try again later"))
//                .build();

        return route("fallbackRoute")
                .route(RequestPredicates.path("/fallback"), request ->
